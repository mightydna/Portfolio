<header id="header">  
    <p id="logo"><a href="#top">DOMINIK</a></p>
    <nav id="menu">
        <ul>
            <li><div class="menu_box" id="menu_btn_about"><a href="#about">Über Mich</a></div></li>
            <li><div class="menu_box"><a href="#skills">Meine Skills</a></div></li>
            <li><div class="menu_box"><a href="#projects">Meine Projekte</a></div></li>
            <li><div class="menu_box"><a href="#experience">Meine Erfahrung</a></div></li>
            <li><div class="menu_box" id="menu_btn_contact"><a href="#contact">Kontakt</a></div></li>
            <li id="lang_select_container"><div class="menu_box">DE</div><div class="menu_box">|</div><div class="menu_box">EN</div></li>
        </ul>
    </nav>
    <!-- Burgermenu -->
    <nav id="burgermenu">
        <div id="burgermenu_main">
            <img src="resources/icons/burger-menu-icon.svg" id="header_burger_icon" alt="Burgermenü-Icon">
        </div>
    </nav>
    <div id="burgermenu_overlay" >
        <div id="bmenu_overlay_close_container">
            <img src="resources/icons/git-icon.svg" id="bmenu_overlay_close_btn" alt="Schliessen-Icon">
        </div>
        <p id="logo">DOMINIK</p>
        <div id="burgermenu_list_container">
            <p>Wo möchtest du hin?</p>
            <ul id="burgermenu_list">
                <li><div class="burger_menu_box" id="burger_menu_btn_about"><a href="#about">Über Mich</a></div></li>
                <li><div class="burger_menu_box" id="burger_menu_btn_skills"><a href="#skills">Meine Skills</a></div></li>
                <li><div class="burger_menu_box" id="burger_menu_btn_projects"><a href="#projects">Meine Projekte</a></div></li>
                <li><div class="burger_menu_box" id="burger_menu_btn_experience"><a href="#experience">Meine Erfahrung</a></div></li>
                <li><div class="burger_menu_box" id="burger_menu_btn_contact"><a href="#contact">Kontakt</a></div></li>
            </ul>
        </div>
        <div id="bmenu_language_menu">
            <p>Sprache ändern?</p>
            <div id="bmenu_language_menu_select">
                <p>DE</p>
                <p>|</p>
                <p>EN</p>
            </div>
        </div>
        <div id="bmenu_b2t_container">
            <div id="bmenu_b2t_bar"></div>
            <div id="bmenu_b2t"><a href="#top">Zurück zum Anfang?</a></div>
        </div>
    </div>
</header>