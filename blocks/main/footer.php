<footer id="footer">
    <div id="footer_bar"></div>
    <div id="footer_elements_container">
        <div id="footer_logo_block">
            <img src="resources/icons/logo_turquoise.svg" alt="Website Icon" id="footer_logo_icon">
            <p id="footer_logo">DOMINIK</p>
        </div>
        <p id="footer_copyright_text">© Dominik <?php echo date("Y"); ?></br><span id="Impressum">Impressum</span></p>
        <a href="#top"><img src="resources/icons/arrow-up-icon.svg" id="footer_b2t" alt="Pfeil-nach-Oben-Icon"></a>
    </div>
    <!-- Responsive Footer -->
    <div id="footer_elements_container_small">
        <div id="footer_logo_container_small">
            <div id="footer_logo_block_small">
                <img src="resources/icons/logo_turquoise.svg" alt="Website Icon" id="footer_logo_icon_small">
                <p id="footer_logo_small">DOMINIK</p>
            </div>
            <p id="footer_copyright_text_small">© Dominik <?php echo date("Y"); ?></br><span  id="Impressum_small">Impressum</span></p>
        </div>
        <a href="#top"><img src="resources/icons/arrow-up-icon.svg" id="footer_b2t_small" alt="Pfeil-nach-Oben-Icon"></a>
    </div>
    <!-- Impressum Overlay -->
    <div id="imprint_overlay">
        
        <div id="imprint_overlay_content_container">    
            
            <div id="imprint_overlay_content_text_container">
                <div id="imprint_overlay_close_container">
                    <h2>Impressum</h2>
                    <img src="resources/icons/icons8-löschen.svg" alt="Schliessen Icon" id="imprint_overlay_close_btn">
                </div>
                <p>Dominik Nagel<br />
                Carl-Peter-Stra&szlig;e 3<br />
                76297 Stutensee</p>

                <h2>Kontakt</h2>
                <p>E-Mail: webmaster@nageldominik.de</p>

                <p>Quelle: <a href="https://www.e-recht24.de/impressum-generator.html">https://www.e-recht24.de/impressum-generator.html</a></p>
            </div>
        </div>
        
    </div>
</footer>